...
type ColumnType = ColumnDef<EpisodeInterface, unknown>;
...
export function EpisodeListPage(props: EpisodeListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EpisodeInterface>>(
    () => `/episodes?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: EpisodeInterface) => {
    if (hasAccess('episode', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/episodes/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'episode_number', header: 'episode_number', accessorKey: 'episode_number' },
    { id: 'title', header: 'Title', accessorKey: 'title' },
    { id: 'duration', header: 'duration', accessorKey: 'duration' },
    hasAccess('season', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'season',
          header: 'Season',
          accessorKey: 'season',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/seasons/view/${record.season?.id}`}>
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/episodes/view/${record.id}`} passHref legacyBehavior>
            <Button
