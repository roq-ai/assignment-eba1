...
import { getMovies, deleteMovieById } from 'apiSdk/movies';
  ...
  const fetcher = useCallback(
    async () =>
      getMovies({
        ...
        order: params.order,
        searchTermKeys: ['title.contains', 'genre.contains', 'director.contains', 'description.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MovieInterface>>(
    () => `/movies?params=${JSON.stringify(params)}`,
    fetcher,
  );
