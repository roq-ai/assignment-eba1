...
import { getMovieById, updateMovieById } from 'apiSdk/movies';
...
function MovieEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<MovieInterface>(
    () => (id ? `/movies/${id}` : null),
    () => getMovieById(id),
  );
  ...
  const handleSubmit = async (values: MovieInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateMovieById(id, values);
      mutate(updated);
      resetForm();
      router.push('/movies');
    } catch (error) {
